...
def feasible_appends(current_positions, map):
    return [
        [position, cost] 
        for position, cost in current_positions 
        if 0 <= position[1] < 500 and 0 <= position[0] < 1200 
        and (map[(499 - position[1]), position[0]] == [0, 0, 0]).all()
    ]

def eig_Neigh(node, map):
    ...
    nxt_neigh = feasible_appends(con_neigh_list, map)
    return nxt_neigh